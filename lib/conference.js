...
Conference.prototype.checkForInfluencerExpiration = function(done) {
    var influencerIndexesToDestroy = [];
    var range = 6;
    var count = 0;
    var limit = 10;
    ...
    for (var i = 0; i < this.influencers.length; ++i) {
        ...
        for (var j = 0; j < this.people.length; ++j) {
            var person = this.people[j];

            var d = vector.VectorDistance(influencer.location, person.location);
            if (d < range) {
                count = count + 1;
            }
        }

        if (count >= limit && this.influencers[i].type[0] != 'x') {
            this.influencers[i].type = 'x' + this.influencers[i].type;
        }
    }
...
Conference.prototype.step = function(done) {
    ...
    this.checkForInfluencerExpiration(function() {
...
Conference.prototype.move = function(data, done) {
    var i = 0;
    for (i = 0; i < this.influencers.length; ++i) {
        if (this.influencers[i].id === data.id) {
            break;
        }
    }

    var point = convertToEPSG3857(data.lng, data.lat);
    var newLocation = new vector(point.x, point.y);
    this.influencers[i].location = newLocation;
    return done();
}
