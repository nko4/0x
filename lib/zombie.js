...
function getRandomFromNoise(z, min, max) {
    if(z.noiseIndex++ >= z.noise.length) {
        z.noiseIndex = 0;
    }

    var n = z.noise[z.noiseIndex];
...
function random(z, people, done) {
    ...
    var force = new vector(getRandomFromNoise(z, -1 * max, max), getRandomFromNoise(z, -1 * max, max));
