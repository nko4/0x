...
function random(p, people, influencers, done) {
    ...
    if(Math.random() > 0.4) {
        force = new vector(utils.getRandomFromNoise(-1 * max, max), utils.getRandomFromNoise(-1 * max, max));
    }
...
function separate(p, people, influencers, done) {
   return p.separate(people, 10, true, done); 
}

function runAwayFromZombies(p, people, influencers, done) {
    return p.separate(p, 1000, false, function(e, force) {
        return done(e, force.mult(0.9));
    });
};
